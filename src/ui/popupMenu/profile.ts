...
export default class PopupMenuProfile extends PopupMenu.PopupSubMenuMenuItem {
    static {
        GObject.registerClass({
            ...
            Signals: {
                "profile-activate": { param_types: [PopupMenu.PopupMenuItem.$gtype, Clutter.Event.$gtype] },
            }
        }, this);
    }
    ...
    #populate_profiles() {
        ...
        for (let profile_name in this.profiles.deepUnpack<SettingsProfileType>()) {
            ...
            profile.connect('activate', (element: PopupMenu.PopupMenuItem, event: any) =>
    ...
    #update_selected_item(old_value: string|null, new_value: string|null) {
        let found_new = false, found_old = false;
        ...
        for (const menu_item of this.menu._getMenuItems() as PopupMenu.PopupMenuItem[]) {
            ...
            if ((!old_value || found_old) && (!new_value || found_new))
                break;
        }
    }
}
