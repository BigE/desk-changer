...
export default class GameMode extends GObject.Object {
    ...
    _onClientCountChanged(proxy: Gio.DBusProxy, properties: GLib.Variant) {
        const unpacked =
            properties.deep_unpack<Record<'ClientCount', GLib.Variant>>();
        if (!('ClientCount' in unpacked)) return;
    ...
    _onProxyReady(_o: Gio.DBusProxy | null, res: Gio.AsyncResult) {
        try {
            ...
            this.#logger?.error(
                'failed to create proxy for GameMode: {}'.format(e)
            );
            return;
        }

        this.#client_count_changed_id = this.#proxy?.connect(
            'g-properties-changed',
            this._onClientCountChanged.bind(this)
        );
