		...
		if self._settings.random is True:
			while True:
				...
				if len(self._prev) > 0 and len(self._prev) >= len(self._wallpapers):
					self._logger.warn(
						'Your history is larger than your available wallpapers. Please consider decreasing the history size or adding more wallpapers to keep things more random.');
					break
